...
const UploadWidget = ({setLink, setId = () => {}, allowedFiles, uploaded, setUploaded, text = "Upload file"}) => {
    ...
    return (
        ...
        <div className={`${isUploaded ? "hidden" : ""}` }>
            <span
                className={`w-full inline-flex justify-center divide-gray-300 overflow-hidden rounded-md px-2 border border-gray-300 bg-gray-50 shadow-sm min-w-25 hover:bg-gray-50 hover:text-gray-900`}
                onClick={() => {widgetRef.current.open()}}
            >
                <button
                type="button"
                className="py-1 sm:py-2 px-2
                 text-sm font-medium text-gray-700 transition-colors focus:relative"
